...
test('deleting a default Service Address promotes the next address or clears canonical profile state',async()=>{
 ...
 assert.match(route,/if\(next\.data\)await setDefault\([^;]+next\.data\.id\);else await syncDefault\([^;]+user\.id,null\)/);
 assert.match(route,/type ProfileAddressSync=/);
 assert.match(route,/const profileValues=profileAddressSync\(address\)/);
 assert.match(route,/await syncDefault\(client,user\.id,null\)/);
