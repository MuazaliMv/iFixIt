...
test('admin navigation exposes Service Categories management',async()=>{
 const roleMenu=await read('app/GlobalRoleMenu.tsx');
 const legacyNav=await read('app/admin/AdminNav.tsx');
 assert.match(roleMenu,/href:'\/admin\/service-categories',label:'Service Categories'/);
 assert.match(legacyNav,/return null;/);
});
...
test('service categories page provides admin CRUD controls',async()=>{
 ...
 assert.match(page,/action:editing\?'update':'create'/);
