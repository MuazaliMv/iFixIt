...
const menus:Record<Role,RoleMenu>={
  admin:{label:'Admin',roleLabel:'System Administrator',home:'/admin',secondary:{href:'/admin/reports',label:'Reports'},sections:[
    ...
    {title:'MANAGEMENT & OPERATIONS',items:[{href:'/admin/requests',label:'Request Management',icon:'requests'},{href:'/admin/escalations',label:'Attention / Escalations',icon:'alert',badge:3},{href:'/admin/service-categories',label:'Service Categories',icon:'services'},{href:'/admin/locations',label:'Locations',icon:'location'},{href:'/admin/users',label:'User Management',icon:'users'}]},
...
function itemIsActive(path:string,href:string){if(href==='/')return path==='/';if(href==='/admin/users')return path.startsWith('/admin/users')||path.startsWith('/admin/providers');return path===href||path.startsWith(href+'/');}
