...
export default function GlobalRoleMenu(){
  const path=usePathname();const[role,setRole]=useState<Role|null>(routeRole(path));const[open,setOpen]=useState(false);
  useEffect(()=>{let active=true;async function resolve(){const byRoute=routeRole(path);if(byRoute){if(active)setRole(byRoute);return;}if(isPublicOrAuth(path)){if(active)setRole(null);return;}try{const{data}=await supabase.auth.getSession();if(!data.session){if(active)setRole(null);return;}const r=await fetch('/api/user/profile',{headers:{Authorization:`Bearer ${data.session.access_token}`}});if(!r.ok){if(active)setRole(null);return;}const p=await r.json();const raw=String(p?.profile?.role||'CUSTOMER').toUpperCase();if(raw==='PROVIDER'&&path==='/profile'){window.location.replace('/provider/profile');return;}if(active)setRole(raw==='ADMIN'?'admin':raw==='PROVIDER'?'provider':'customer');}catch{if(active)setRole(null);}}void resolve();setOpen(false);return()=>{active=false;};},[path]);
