...
export default function MobileNav({role}:Props){
 ...
 const provider:Item[]=[{href:'/provider/today',label:'Today',icon:'⌂',match:p=>p==='/provider/today'||p==='/provider/jobs'||p==='/provider'},{href:'/provider/calendar',label:'Calendar',icon:'□',match:p=>p==='/provider/calendar'},{href:'/provider/listings',label:'Listings',icon:'▤',match:p=>p==='/provider/listings'||p==='/provider/setup'||p==='/provider/onboarding'},{href:'/messages',label:'Messages',icon:'◌',match:p=>p==='/messages'},{href:'/provider/menu',label:'Menu',icon:'☰',match:p=>p==='/provider/menu'||p.startsWith('/provider/earnings')}];
