...
function preferred(v:string){const d=new Date(`${v}T00:00:00`);return Number.isNaN(d.getTime())?v:d.toLocaleDateString(undefined,{day:'numeric',month:'short',year:'numeric'});}
...
function dispatchText(d:DispatchRow|undefined,now:number){
 if(!d)return null;
 const count=Math.min(5,d.available_provider_count||0);
 const searchTimer=countdown(searchDeadline(d),now);
 const responseTimer=countdown(d.dispatch_customer_response_deadline_at,now);
 if(d.dispatch_state==='SEARCHING'){
  if(d.dispatch_customer_mode==='WAITING_MORE')return{title:`Waiting for more providers · ${count}/5`,body:'We are keeping your request open to find more available providers.',timer:searchTimer,tone:'search'};
  if(count>0)return{title:`${count} provider${count===1?'':'s'} available`,body:'You can choose now or keep searching for more options.',timer:responseTimer,tone:'secured'};
  return{title:'Searching for available providers',body:'This request is scheduled and will be sent to matching providers.',timer:searchTimer,tone:'search'};
 }
 if(d.dispatch_state==='EXTENDED'){
  if(d.dispatch_customer_mode==='WAITING_MORE')return{title:`Waiting for more providers · ${count}/5`,body:'The provider search has been extended.',timer:searchTimer,tone:'extended'};
  if(count>0)return{title:`${count} provider${count===1?'':'s'} available`,body:'Choose now or wait while the extended search continues.',timer:responseTimer,tone:'secured'};
  return{title:'Search extended by 1 hour',body:'We are continuing to look for an available provider.',timer:searchTimer,tone:'extended'};
 }
 if(d.dispatch_state==='AWAITING_CUSTOMER')return{title:`Choose from ${count} available provider${count===1?'':'s'}`,body:'Provider search is complete. Select a provider before the timer expires.',timer:responseTimer,tone:'secured'};
 if(d.dispatch_state==='CUSTOMER_TIMEOUT')return{title:'Provider selection timed out',body:"We couldn't complete provider assignment because we didn't receive your response after 3 retries.",timer:null,tone:'exhausted'};
 if(d.dispatch_state==='EXHAUSTED')return{title:'No providers available',body:NO_PROVIDER_MESSAGE,timer:null,tone:'exhausted'};
 if(d.dispatch_state==='SECURED')return{title:'Provider assigned',body:'A service provider has accepted your request. Open the request to continue.',timer:null,tone:'secured'};
 return null;
}
...
export default function MyRequestsPage(){
 ...
 return <main className="c3Page"><CustomerHeader title="Requests" backHref="/" right={<a className="c3IconButton" href="/?new=1" aria-label="New request"><svg viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg></a>}/><div className="c3Shell c3Requests">
  <div className="c3RequestsTop"><div><h1>My requests</h1><p>{counts.active?`${counts.active} active request${counts.active===1?'':'s'}`:'Nothing needs your attention right now.'}</p></div><button className="c3Secondary c3Refresh" onClick={()=>void load()} disabled={busy}>{busy?'Refreshing…':'Refresh'}</button></div>
  ...
  <div className="c3Filters"><button className={filter==='ACTIVE'?'active':''} onClick={()=>setFilter('ACTIVE')}><span className="c3FilterIcon">▣</span>Active {counts.active}</button><button className={filter==='COMPLETED'?'active':''} onClick={()=>setFilter('COMPLETED')}><span className="c3FilterIcon">✓</span>Completed {counts.completed}</button><button className={filter==='ALL'?'active':''} onClick={()=>setFilter('ALL')}><span className="c3FilterIcon">⊞</span>All {counts.all}</button></div>
  {message&&message!=='Up to date'?<div className="c3Notice">{message}</div>:null}
  <section className="c3RequestList">{visible.map(r=>{const d=dispatch[r.ticket_number];const secured=Boolean(r.assigned_provider_label||d?.assigned_provider_user_id||d?.dispatch_state==='SECURED');const info=dispatchText(d,now);const retryable=['EXHAUSTED','CUSTOMER_TIMEOUT'].includes(String(d?.dispatch_state));const canWait=Boolean(!secured&&d&&['SEARCHING','EXTENDED'].includes(String(d.dispatch_state))&&d.available_provider_count>0&&d.available_provider_count<5&&d.dispatch_customer_mode!=='WAITING_MORE');return <article key={r.id} className={`c3RequestCard ${r.status==='COMPLETED'?'completed':''}`}>
   <div className="c3RequestMain"><div className="c3RequestIcon"><ServiceIcon name={r.service_name}/></div><div className="c3RequestIdentity"><span className={`c3Status ${r.status==='COMPLETED'?'completed':''}`}>{pretty(secured&&r.status==='PENDING'?'ACCEPTED':r.status)}</span><h2>{r.service_name}</h2><p className="c3ProviderLine"><span aria-hidden="true">♙</span>{r.status==='CANCELLED'?'Cancelled before provider selection':r.assigned_provider_label||(secured?'Provider assigned':'Provider not selected yet')}</p><p className="c3Ticket">Request ID: <strong>{r.ticket_number}</strong></p></div></div>
   {info?<div className={`c3SearchPanel ${info.tone}`}><div className="c3SearchPulse" aria-hidden="true"><span></span></div><div className="c3SearchCopy"><strong>{info.title}</strong><p>{info.body}</p>{d?.dispatch_attempt>1?<small>Search attempt {d.dispatch_attempt}</small>:null}</div>{info.timer?<div className="c3Countdown"><span>Live countdown</span><strong>{info.timer}</strong><small>HRS&nbsp;&nbsp;&nbsp;&nbsp; MIN&nbsp;&nbsp;&nbsp;&nbsp; SEC</small></div>:null}</div>:null}
   <div className="c3RequestMeta"><div><span className="c3MetaIcon">●</span><div><small>Location</small><strong>{r.service_location_text}</strong></div></div><div><span className="c3MetaIcon">□</span><div><small>Preferred date</small><strong>{preferred(r.preferred_date)}</strong></div></div><div><span className="c3MetaIcon">＋</span><div><small>Created on</small><strong>{when(r.created_at)}</strong></div></div></div>
   <div className="c3NextPanel"><div className="c3NextIcon" aria-hidden="true">⌛</div><div><small>Next step</small><strong>{secured?'Continue with assigned provider':retryable?'Retry provider search or cancel the request':d?.dispatch_customer_mode==='WAITING_MORE'?'Waiting for another provider':d?.available_provider_count>0?'Provider responses received':actionFor(r)}</strong><p>{secured?'A provider has accepted your request. Open the request for the latest details.':r.status==='PENDING'?'We’ll notify you as soon as a provider responds.':'Open the request for the latest status and available actions.'}</p></div></div>
   <div className="c3RequestAction"><div className="c3ActionButtons">{retryable?<button className="c3Primary" type="button" disabled={retrying===r.ticket_number} onClick={()=>void retrySearch(r)}>{retrying===r.ticket_number?'Restarting…':'Try Again'}</button>:null}{canWait?<button className="c3Secondary" type="button" disabled={waiting===r.ticket_number} onClick={()=>void waitForMore(r)}>{waiting===r.ticket_number?'Waiting…':'Wait for More Providers'}</button>:null}{!secured&&d?.dispatch_customer_mode==='WAITING_MORE'?<button className="c3Secondary" type="button" disabled>Waiting for More</button>:null}{!secured&&canCancel(r)?<button className="c3Danger" type="button" disabled={cancelling===r.ticket_number} onClick={()=>void cancelRequest(r)}><span aria-hidden="true">♲</span>{cancelling===r.ticket_number?'Cancelling…':'Cancel Request'}</button>:null}<a className="c3Primary c3OpenDetails" href={`/requests/${encodeURIComponent(r.ticket_number)}`}>{secured?'Open Provider':'Open Details'}<span aria-hidden="true">›</span></a></div></div>
   {!secured&&r.status==='PENDING'?<div className="c3NotifyStrip"><span className="c3Shield">✓</span><div><strong>We’re finding the best provider for you.</strong><p>You’ll be notified as soon as the status changes.</p></div><span className="c3Bell" aria-hidden="true">●</span></div>:null}
  </article>})}{!visible.length?<div className="c3Empty"><h2>No requests here</h2><p>{filter==='COMPLETED'?'Completed requests will appear here.':'Create a request when you need a local service provider.'}</p><a className="c3Primary" href="/">Create request</a></div>:null}</section>
 </div></main>;
}