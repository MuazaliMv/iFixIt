...
export default function DispatchLivePanel(){
 ...
 const title=state==='AWAITING_CUSTOMER'?`Choose a provider · ${count}/5`:waiting?`Waiting for more providers · ${count}/5`:count>0?`${count} provider${count===1?'':'s'} available`:`Searching · ${row.dispatch_tier||'STANDARD'}`;
 const subtitle=state==='AWAITING_CUSTOMER'?`Response countdown ${responseTimer} · retries ${row.dispatch_customer_retry_count||0}/3`:waiting?`Search countdown ${searchTimer}`:count>0?`Response ${responseTimer} · search ${searchTimer}`:`Live countdown ${searchTimer}`;
 if(['EXHAUSTED','CUSTOMER_TIMEOUT','CANCELLED','NOT_REQUIRED','SECURED'].includes(String(state)))return null;
 return <aside className="dispatchLive" aria-live="polite"><div><span className="dispatchLiveEyebrow">Live Dispatch</span><strong>{title}</strong><small>{subtitle}</small>{note?<small className="dispatchLiveNote">{note}</small>:null}</div><div className="dispatchLiveActions">{canWait?<button type="button" onClick={()=>void waitMore()} disabled={busy}>{busy?'Updating…':'Wait for More'}</button>:null}{waiting?<button type="button" disabled>Waiting for More</button>:null}{count>0?<button className="primary" type="button" onClick={()=>document.querySelector('.providerGrid')?.scrollIntoView({behavior:'smooth',block:'center'})}>Select Now</button>:null}</div></aside>;
}
