...
type Row={ticket_number:string;status:string;dispatch_tier?:'URGENT'|'STANDARD'|'SCHEDULED'|null;dispatch_state?:DispatchState|null;dispatch_initial_deadline_at?:string|null;dispatch_extension_deadline_at?:string|null;available_provider_count:number;assigned_provider_user_id?:string|null};
...
export default function DispatchLivePanel(){
 ...
 const[row,setRow]=useState<Row|null>(null);const[now,setNow]=useState(()=>Date.now());const[position,setPosition]=useState<Position|null>(null);const[dragging,setDragging]=useState(false);const drag=useRef<DragState>(null);const wasUnassigned=useRef(false);const reloading=useRef(false);
 const call=useCallback(async(body:Record<string,unknown>)=>{const{data}=await supabase.auth.getSession();if(!data.session)return null;const response=await fetch(DISPATCH_API,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${data.session.access_token}`},body:JSON.stringify(body)});const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload?.error||'Unable to check provider search');return payload;},[]);
 const journeyAccess=useCallback(async()=>{const{data}=await supabase.auth.getSession();if(!data.session||!ticket)return{owner:false,selected:false};const response=await fetch(MARKET_API,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${data.session.access_token}`},body:JSON.stringify({action:'journey',ticketNumber:ticket})});if(!response.ok)return{owner:false,selected:false};const payload=await response.json().catch(()=>({}));const journeyTicket=String(payload?.request?.ticket_number||'').toUpperCase();return{owner:journeyTicket===ticket,selected:Boolean(payload?.selectedProvider)};},[ticket]);
 const refresh=useCallback(async()=>{if(!ticket)return;try{const[p,access]=await Promise.all([call({action:'status'}),journeyAccess()]);if(!access.owner){setRow(null);return;}const found=(p?.requests||[]).find((r:Row)=>r.ticket_number===ticket)||null;if(found){const assigned=access.selected||Boolean(found.assigned_provider_user_id)||found.dispatch_state==='SECURED'||['ACCEPTED','INSPECTION_SCHEDULED','PROCESSING','IN_PROGRESS','COMPLETED'].includes(String(found.status).toUpperCase());if(assigned&&wasUnassigned.current&&!reloading.current){reloading.current=true;window.location.reload();return;}if(!assigned)wasUnassigned.current=true;if(access.selected&&!found.assigned_provider_user_id){setRow({...found,assigned_provider_user_id:'__selected_provider__'});return;}}setRow(found);}catch{setRow(null);}},[ticket,call,journeyAccess]);
 ...
 if(['EXHAUSTED','CANCELLED','NOT_REQUIRED','SECURED'].includes(String(state)))return null;
 const deadline=state==='EXTENDED'?row.dispatch_extension_deadline_at:row.dispatch_initial_deadline_at;
 const timerValue=timer(deadline,now);
 const legacyFinalizing=state==='AWAITING_CUSTOMER'||state==='CUSTOMER_TIMEOUT';
 const title=legacyFinalizing?'Finalizing provider assignment':'Finding a service provider';
 const subtitle=legacyFinalizing?'A provider response from the previous flow is being reconciled automatically. You do not need to choose or confirm a provider.':'Matching approved providers are being contacted in ranked order. The first eligible provider who accepts is assigned automatically.';
 return <aside className={`dispatchLive ${dragging?'isDragging':''}`} aria-label="Live Dispatch" aria-live="polite" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} style={position?{left:position.left,top:position.top,bottom:'auto',transform:'none'}:undefined}>
  ...
  <div className="dispatchLiveBody"><span className="dispatchLiveEyebrow">Live provider search</span><strong>{title}</strong><small>{subtitle}</small>{!legacyFinalizing?<div className="dispatchLiveTimer"><span>Search window</span><b>{timerValue}</b></div>:null}</div>
 </aside>;
}
