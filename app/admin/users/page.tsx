...
type UserRow={user_id:string;email?:string|null;full_name?:string|null;role:'CUSTOMER'|'PROVIDER'|'ADMIN';provider_approved:boolean;created_at:string;updated_at:string};
...
export default function AdminUsersPage(){
 ...
 async function load(){try{const payload=await callAdmin({action:'dashboard'});if(!payload)return;setUsers(payload.users||[]);setMessage('User list loaded.');}catch(error){setMessage(error instanceof Error?error.message:'Unable to load users.');}}
 async function changeRole(row:UserRow,role:'CUSTOMER'|'PROVIDER'){setBusyUser(row.user_id);try{await callAdmin({action:'set_user_role',targetUserId:row.user_id,role,providerApproved:role==='PROVIDER'?row.provider_approved:false});await load();setMessage(`${row.full_name||row.email||'User'} changed to ${role}.`);}catch(error){setMessage(error instanceof Error?error.message:'Unable to change role.');}finally{setBusyUser('');}}
