...
export default function AdminUsersPage(){
 ...
 return <main className="shell"><header className="topbar"><div><a className="brand" href="/admin">FixIt</a><p className="tagline">Admin • Users</p></div><button className="secondary" onClick={()=>void load()}>Refresh</button></header><AdminNav/>
  <section className="panel"><div className="panelHeader"><div><p className="eyebrow">IDENTITY & ACCESS</p><h2>User Accounts</h2><p className="muted">Search, review and manage user access.</p></div><span className="pill">{users.length} users</span></div>
  ...
  <section className="panel" style={{display:'grid',gap:14}}>{visible.map(u=>{
   const open=!!expanded[u.user_id];const lastActive=u.last_seen_at||u.last_sign_in_at;const status=u.account_status||u.status||'Active';
   return <article key={u.user_id} style={{border:'1px solid var(--border)',borderRadius:20,padding:18,background:'var(--surface)',display:'grid',gap:14}}>
    <div style={{display:'flex',gap:13,alignItems:'center'}}>{avatar(u)}<div style={{minWidth:0,flex:1}}><div style={{display:'flex',gap:7,alignItems:'center',flexWrap:'wrap'}}><h3 style={{margin:0,fontSize:20}}>{u.full_name||'Unnamed user'}</h3>{u.user_id===currentUserId?<span className="selfTag">You</span>:null}</div><p className="muted" style={{margin:'3px 0 7px',overflowWrap:'anywhere'}}>{u.email||'No email address'}</p><div style={{display:'flex',gap:6,flexWrap:'wrap'}}><span className="pill">{u.role}</span><span className="pill">{status}</span>{u.role==='PROVIDER'?<span className={u.provider_approved?'profileBadge verified':'profileBadge warning'}>{u.provider_approved?'Approved':'Pending'}</span>:null}</div></div></div>
