...
export default function AdminUsersPage(){
 ...
 function avatar(u:UserRow){const src=u.photoUrl||(/^https?:\/\//i.test(u.profile_photo_url||'')?u.profile_photo_url:null);const initial=(u.full_name||u.email||'U').slice(0,1).toUpperCase();return src?<img src={src} alt={`${u.full_name||'User'} profile`} onError={e=>{e.currentTarget.style.display='none';const fallback=e.currentTarget.nextElementSibling as HTMLElement|null;if(fallback)fallback.style.display='inline-grid';}} style={{width:64,height:64,borderRadius:'50%',objectFit:'cover',flex:'0 0 auto',border:'1px solid var(--border)'}}/>:null||<span style={{width:64,height:64,borderRadius:'50%',display:src?'none':'inline-grid',placeItems:'center',background:'var(--surface-alt)',fontWeight:800,fontSize:22,flex:'0 0 auto'}}>{initial}</span>;}
