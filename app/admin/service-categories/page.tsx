...
type Category={
 id:string;code:string;name:string;description:string|null;is_active:boolean;created_at:string;updated_at:string;
};
type FormState={id:string|null;name:string;description:string;isActive:boolean};
const blank:FormState={id:null,name:'',description:'',isActive:true};
...
export default function ServiceCategoriesPage(){
 ...
 useEffect(()=>{void load();},[]);
 const filtered=useMemo(()=>{const q=query.trim().toLowerCase();return q?categories.filter(c=>`${c.name} ${c.description||''}`.toLowerCase().includes(q)):categories;},[categories,query]);
 ...
 function beginEdit(category:Category){setForm({id:category.id,name:category.name,description:category.description||'',isActive:category.is_active});setOpen(true);setMessage('');}
 async function save(event:FormEvent){event.preventDefault();if(!form.name.trim()){setMessage('Category name is required.');return;}setSaving(true);try{await call({action:form.id?'update':'create',id:form.id,name:form.name,description:form.description,isActive:form.isActive});setOpen(false);setForm(blank);await load();setMessage(form.id?'Service category updated.':'Service category created.');}catch(error){setMessage(error instanceof Error?error.message:'Unable to save category.');}finally{setSaving(false);}}
 async function remove(category:Category){if(!window.confirm(`Delete “${category.name}”? This cannot be undone.`))return;setSaving(true);try{await call({action:'delete',id:category.id});await load();setMessage('Service category deleted.');}catch(error){setMessage(error instanceof Error?error.message:'Unable to delete category.');}finally{setSaving(false);}}

 return <>
  <AdminNav/>
  <main className="categoryAdminPage">
   <section className="categoryAdminHead">
    <div><span className="eyebrow">MANAGEMENT & OPERATIONS</span><h1>Service Categories</h1><p>Manage the categories customers and service providers use across FixIt.</p></div>
