...
export default function ServiceCategoriesPage(){
 ...
 const matchesQuery=(category:Category,q:string)=>`${category.name} ${category.description||''} ${category.code}`.toLowerCase().includes(q);
 const branchMatches=(category:Category,q:string):boolean=>matchesQuery(category,q)||(childrenByParent.get(category.id)||[]).some(child=>branchMatches(child,q));
 const filtered=useMemo(()=>{
  const q=query.trim().toLowerCase();
  return q?topLevel.filter(category=>branchMatches(category,q)):topLevel;
 ...
 function toggleCategory(category:Category){if(!(childrenByParent.get(category.id)||[]).length)return;setExpandedCategoryIds(current=>{const next=new Set(current);if(next.has(category.id))next.delete(category.id);else next.add(category.id);return next;});}
 function stop(event:MouseEvent){event.stopPropagation();}
 async function save(event:FormEvent){event.preventDefault();if(!form.name.trim()){setMessage(form.parentId?'Subcategory name is required.':'Category name is required.');return;}setSaving(true);try{const editing=Boolean(form.id);const subcategory=Boolean(form.parentId);await call({action:editing?'update':'create',id:form.id,parentId:form.parentId,name:form.name,description:form.description,isActive:form.isActive});setOpen(false);setForm(blank);await load();setMessage(editing?(subcategory?'Subcategory updated.':'Service category updated.'):(subcategory?'Subcategory created.':'Service category created.'));}catch(error){setMessage(error instanceof Error?error.message:'Unable to save category.');}finally{setSaving(false);}}
 async function remove(category:Category){if(!window.confirm(`Delete “${category.name}”? This cannot be undone.`))return;setSaving(true);try{await call({action:'delete',id:category.id});await load();setExpandedCategoryIds(current=>{const next=new Set(current);next.delete(category.id);return next;});setMessage(category.parent_id?'Subcategory deleted.':'Service category deleted.');}catch(error){setMessage(error instanceof Error?error.message:'Unable to delete category.');}finally{setSaving(false);}}
 ...
 return <>
  ...
  <main className="categoryAdminPage">
   <section className="categoryAdminHead">
    <div><span className="eyebrow">MANAGEMENT & OPERATIONS</span><h1>Service Categories</h1><p>Click a category to view its existing subcategories. Use the action buttons to add another level, edit, or delete.</p></div>
   ...
   <div className="categoryTableWrap"><table className="categoryTable"><thead><tr><th>Category Name</th><th>Description</th><th>Status</th><th>Created Date</th><th>Actions</th></tr></thead><tbody>{filtered.map(category=><CategoryBranch key={category.id} category={category} depth={0} query={query} childrenByParent={childrenByParent} expandedCategoryIds={expandedCategoryIds} toggleCategory={toggleCategory} saving={saving} beginSubcategory={beginSubcategory} beginEdit={beginEdit} remove={remove} stop={stop}/>)}</tbody></table></div>}
  </main>
  ...
  <style jsx global>{`
   .categoryAdminPage{max-width:1180px;margin:0 auto;padding:28px 22px 90px;color:#0f172a}.categoryAdminHead{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:20px}.eyebrow{display:block;color:#2563eb;font-size:11px;font-weight:900;letter-spacing:.11em}.categoryAdminHead h1,.categoryModal h2{margin:6px 0 5px;font-size:clamp(28px,5vw,42px);letter-spacing:-.04em}.categoryAdminHead p{margin:0;color:#64748b;max-width:680px}.primaryAction{border:0;border-radius:12px;background:#2563eb;color:#fff;padding:11px 16px;font-weight:850;cursor:pointer;white-space:nowrap}.primaryAction:disabled{opacity:.55;cursor:not-allowed}.categoryToolbar{display:flex;align-items:center;gap:12px;margin-bottom:14px}.categoryToolbar input{flex:1;min-height:44px;border:1px solid #dbe2ea;border-radius:12px;padding:0 14px;background:#fff;font:inherit}.categoryToolbar span{color:#64748b;font-size:13px;font-weight:800}.categoryMessage{margin:0 0 14px;padding:12px 14px;border:1px solid #dbeafe;border-radius:12px;background:#eff6ff;color:#1d4ed8;font-weight:750}.categoryEmpty{padding:42px 18px;text-align:center;border:1px dashed #cbd5e1;border-radius:16px;color:#64748b;background:#fff}.categoryTableWrap{overflow:auto;border:1px solid #e2e8f0;border-radius:18px;background:#fff;box-shadow:0 8px 28px rgba(15,23,42,.05)}.categoryTable{width:100%;border-collapse:collapse;min-width:820px}.categoryTable th{padding:13px 16px;text-align:left;background:#f8fafc;color:#64748b;font-size:11px;text-transform:uppercase;letter-spacing:.07em}.categoryTable td{padding:15px 16px;border-top:1px solid #eef2f7;vertical-align:middle}.categoryRow{cursor:pointer}.categoryRow:hover{background:#f8fbff}.categoryRow.noChildren{cursor:default}.categoryTable td:first-child strong,.categoryTable td:first-child small{display:block}.categoryNameLine{display:flex;align-items:center;gap:8px}.categoryNameLine strong{display:inline!important}.expandIndicator{display:inline-grid;place-items:center;width:22px;height:22px;flex:0 0 22px;border-radius:7px;background:#eff6ff;color:#2563eb;font-size:13px;font-weight:900}.expandIndicator.empty{background:#f8fafc;color:#cbd5e1}.childCount{color:#2563eb!important;font-weight:800}.subcategoryBadge{display:inline-flex!important;width:max-content;margin-top:5px!important;padding:2px 7px;border-radius:999px;background:#eef2ff;color:#4f46e5!important;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.04em}.statusPill{display:inline-flex;padding:5px 9px;border-radius:999px;font-size:12px;font-weight:850}.statusPill.active{background:#ecfdf5;color:#15803d}.statusPill.inactive{background:#f1f5f9;color:#64748b}.rowActions{display:flex;gap:7px;flex-wrap:wrap}.rowActions button{border:1px solid #cbd5e1;border-radius:9px;background:#fff;padding:7px 10px;font-weight:800;cursor:pointer}.rowActions .addAction{border-color:#bfdbfe;color:#2563eb;background:#eff6ff}.rowActions .deleteAction{border-color:#fecaca;color:#dc2626;background:#fff7f7}.categoryModalBackdrop{position:fixed;inset:0;z-index:3000;display:grid;place-items:center;padding:18px;background:rgba(15,23,42,.38);backdrop-filter:blur(5px)}.categoryModal{width:min(100%,520px);display:grid;gap:16px;padding:20px;border-radius:20px;background:#fff;box-shadow:0 30px 90px rgba(15,23,42,.28)}.categoryModalHead{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.categoryModal h2{font-size:28px}.categoryModalHead>button{width:38px;height:38px;border:0;border-radius:10px;background:#f1f5f9;color:#64748b;font-size:26px}.parentHint{margin:5px 0 0;color:#64748b;font-size:13px}.categoryModal label{display:grid;gap:7px;color:#334155;font-size:13px;font-weight:850}.categoryModal input[type=text],.categoryModal input:not([type]),.categoryModal textarea{width:100%;border:1px solid #cbd5e1;border-radius:11px;padding:11px 12px;font:inherit;resize:vertical}.toggleRow{grid-template-columns:1fr auto!important;align-items:center;padding:13px 14px;border:1px solid #e2e8f0;border-radius:12px;background:#f8fafc}.toggleRow small{display:block;margin-top:3px;color:#64748b;font-weight:600}.toggleRow input{width:22px;height:22px}.categoryModalActions{display:flex;justify-content:flex-end;gap:8px;padding-top:4px}.categoryModalActions>button:not(.primaryAction){border:1px solid #cbd5e1;border-radius:11px;background:#fff;padding:10px 14px;font-weight:800}
   @media(max-width:700px){.categoryAdminPage{padding:20px 14px 90px}.categoryAdminHead{align-items:stretch;flex-direction:column}.categoryAdminHead .primaryAction{width:100%;min-height:46px}.categoryToolbar{align-items:stretch;flex-direction:column}.categoryToolbar span{padding-left:2px}.categoryTableWrap{overflow:visible;border:0;background:transparent;box-shadow:none}.categoryTable,.categoryTable tbody,.categoryTable tr,.categoryTable td{display:block;min-width:0}.categoryTable thead{display:none}.categoryTable tr{margin-bottom:12px;padding:14px;border:1px solid #e2e8f0;border-radius:16px;background:#fff;box-shadow:0 5px 18px rgba(15,23,42,.05)}.categoryTable tr[data-depth]:not([data-depth='0']){margin-left:min(calc(var(--category-depth) * 12px),36px);border-left:4px solid #dbeafe}.categoryTable td{display:grid;grid-template-columns:115px 1fr;gap:8px;padding:8px 0;border:0}.categoryTable td:before{color:#94a3b8;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.05em}.categoryTable td:nth-child(1):before{content:'Category'}.categoryTable td:nth-child(2):before{content:'Description'}.categoryTable td:nth-child(3):before{content:'Status'}.categoryTable td:nth-child(4):before{content:'Created'}.categoryTable td:nth-child(5):before{content:'Actions'}.rowActions{justify-content:flex-start}.rowActions button{min-height:40px}.categoryModal{max-height:calc(100dvh - 32px);overflow:auto}.categoryModalActions{display:grid;grid-template-columns:1fr 1fr}.categoryModalActions button{min-height:44px}}
  `}</style>
 </>;
}

type CategoryBranchProps={category:Category;depth:number;query:string;childrenByParent:Map<string,Category[]>;expandedCategoryIds:Set<string>;toggleCategory:(category:Category)=>void;saving:boolean;beginSubcategory:(category:Category)=>void;beginEdit:(category:Category)=>void;remove:(category:Category)=>Promise<void>;stop:(event:MouseEvent)=>void};
function CategoryBranch({category,depth,query,childrenByParent,expandedCategoryIds,toggleCategory,saving,beginSubcategory,beginEdit,remove,stop}:CategoryBranchProps){
 const q=query.trim().toLowerCase();
 const children=childrenByParent.get(category.id)||[];
 const hasChildren=children.length>0;
 const isExpanded=Boolean(q)||expandedCategoryIds.has(category.id);
 const branchMatches=(node:Category):boolean=>`${node.name} ${node.description||''} ${node.code}`.toLowerCase().includes(q)||(childrenByParent.get(node.id)||[]).some(branchMatches);
 const visibleChildren=q?children.filter(branchMatches):children;
 return <>
  <tr className={`categoryRow ${hasChildren?'':'noChildren'}`} data-depth={depth} style={{'--category-depth':depth} as React.CSSProperties} onClick={()=>toggleCategory(category)} title={hasChildren?`${isExpanded?'Hide':'View'} subcategories under ${category.name}`:`${category.name} has no subcategories`}>
   <td style={{paddingLeft:depth?`${Math.min(16+depth*22,104)}px`:undefined}}><div className="categoryNameLine"><span className={`expandIndicator ${hasChildren?'':'empty'}`}>{hasChildren?(isExpanded?'▾':'▸'):'•'}</span><strong>{depth?'↳ ':''}{category.name}</strong></div><small>{category.code}</small>{hasChildren?<small className="childCount">{children.length} {children.length===1?'subcategory':'subcategories'} · {isExpanded?'Tap to hide':'Tap to view'}</small>:null}{depth>0?<small className="subcategoryBadge">Subcategory level {depth}</small>:null}</td>
   <td>{category.description||'—'}</td>
   <td><span className={`statusPill ${category.is_active?'active':'inactive'}`}>{category.is_active?'Active':'Inactive'}</span></td>
   <td>{dateLabel(category.created_at)}</td>
   <td><div className="rowActions" onClick={stop}><button type="button" className="addAction" onClick={()=>beginSubcategory(category)}>+ Subcategory</button><button type="button" onClick={()=>beginEdit(category)}>Edit</button><button type="button" className="deleteAction" disabled={saving} onClick={()=>void remove(category)}>Delete</button></div></td>
  </tr>
  {isExpanded?visibleChildren.map(child=><CategoryBranch key={child.id} category={child} depth={depth+1} query={query} childrenByParent={childrenByParent} expandedCategoryIds={expandedCategoryIds} toggleCategory={toggleCategory} saving={saving} beginSubcategory={beginSubcategory} beginEdit={beginEdit} remove={remove} stop={stop}/>):null}
 </>;
}
