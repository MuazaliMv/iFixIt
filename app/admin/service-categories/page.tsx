...
export default function ServiceCategoriesPage(){
 ...
 async function save(event:FormEvent){event.preventDefault();if(!form.name.trim()){setMessage(form.parentId?'Subcategory name is required.':'Category name is required.');return;}setSaving(true);try{const editing=Boolean(form.id);const subcategory=Boolean(form.parentId);await call({action:form.id?'update':'create',id:form.id,parentId:form.parentId,name:form.name,description:form.description,isActive:form.isActive});setOpen(false);setForm(blank);await load();setMessage(editing?(subcategory?'Subcategory updated.':'Service category updated.'):(subcategory?'Subcategory created.':'Service category created.'));}catch(error){setMessage(error instanceof Error?error.message:'Unable to save category.');}finally{setSaving(false);}}
 ...
 return <>
  ...
  <main className="categoryAdminPage">
   <section className="categoryAdminHead">
    <div><span className="eyebrow">MANAGEMENT & OPERATIONS</span><h1>Service Categories</h1><p>Every category and subcategory has the same controls. Add another level, edit, or delete directly from its card.</p></div>
   ...
   <div className="categoryTableWrap"><table className="categoryTable"><thead><tr><th>Category Name</th><th>Description</th><th>Status</th><th>Created Date</th><th>Actions</th></tr></thead><tbody>{filtered.map(category=><CategoryBranch key={category.id} category={category} depth={0} query={query} childrenByParent={childrenByParent} saving={saving} beginSubcategory={beginSubcategory} beginEdit={beginEdit} remove={remove} stop={stop}/>)}</tbody></table></div>}
  </main>
...
type CategoryBranchProps={category:Category;depth:number;query:string;childrenByParent:Map<string,Category[]>;saving:boolean;beginSubcategory:(category:Category)=>void;beginEdit:(category:Category)=>void;remove:(category:Category)=>Promise<void>;stop:(event:MouseEvent)=>void};
function CategoryBranch({category,depth,query,childrenByParent,saving,beginSubcategory,beginEdit,remove,stop}:CategoryBranchProps){
 const q=query.trim().toLowerCase();
 ...
 return <>
  <tr className="categoryRow" data-depth={depth} style={{'--category-depth':depth} as React.CSSProperties} onClick={()=>beginSubcategory(category)} title={`Add subcategory under ${category.name}`}>
   <td style={{paddingLeft:depth?`${Math.min(16+depth*22,104)}px`:undefined}}><strong>{depth?'↳ ':''}{category.name}</strong><small>{category.code}</small>{depth>0?<small className="subcategoryBadge">Subcategory level {depth}</small>:null}</td>
  ...
  {visibleChildren.map(child=><CategoryBranch key={child.id} category={child} depth={depth+1} query={query} childrenByParent={childrenByParent} saving={saving} beginSubcategory={beginSubcategory} beginEdit={beginEdit} remove={remove} stop={stop}/>) }
 </>;
}
