...
export default function AdminPage(){
 ...
 async function loadDashboard(){setLoading(true);try{const payload=await callAdmin({action:'dashboard'});setCounts(payload.counts||counts);const users=payload.users||[];const providers=payload.providers||[];setUserCount(users.length);setProviderCount(providers.length);setPendingProviders(providers.filter((p:{provider_approved:boolean})=>!p.provider_approved).length);setMessage('Admin dashboard refreshed.');}catch(error){setMessage(error instanceof Error?error.message:'Unable to load dashboard.');}finally{setLoading(false);}}
