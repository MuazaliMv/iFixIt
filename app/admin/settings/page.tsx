...
export default function AdminSettingsPage(){
 const[rows,setRows]=useState<ConfigRow[]>([]);const[geo,setGeo]=useState<Geo|null>(null);const[message,setMessage]=useState('Loading configuration…');const[busy,setBusy]=useState(false);
 useEffect(()=>{void load();},[]);
 ...
 async function load(){setBusy(true);try{const p=await call({action:'list'});const cfg=p.config||[];setRows(cfg);const g=cfg.find((r:ConfigRow)=>r.key==='geolocation.provider')?.value||{};setGeo(g as Geo);setMessage('Live configuration loaded.');}catch(e){setMessage(e instanceof Error?e.message:'Unable to load configuration.');}finally{setBusy(false);}}
