...
type LocationUnit={id:string;atoll_id:string;island_id:string;unit_type:string;display_name:string};
...
export default function ServiceAddressManager(){
 const[addresses,setAddresses]=useState<ServiceAddress[]>([]);const[atolls,setAtolls]=useState<Atoll[]>([]);const[islands,setIslands]=useState<Island[]>([]);const[wards,setWards]=useState<LocationUnit[]>([]);
 ...
 const[label,setLabel]=useState('Home');const[house,setHouse]=useState('');const[road,setRoad]=useState('');const[atollId,setAtollId]=useState('');const[islandId,setIslandId]=useState('');const[wardId,setWardId]=useState('');const[accessInstructions,setAccessInstructions]=useState('');
 const filteredIslands=useMemo(()=>islands.filter(i=>i.atoll_id===atollId),[islands,atollId]);
 const filteredWards=useMemo(()=>wards.filter(w=>w.atoll_id===atollId&&w.island_id===islandId),[wards,atollId,islandId]);
 const selectedAtoll=atolls.find(a=>a.id===atollId)||null;const selectedIsland=islands.find(i=>i.id===islandId&&i.atoll_id===atollId)||null;const selectedWard=wards.find(w=>w.id===wardId&&w.island_id===islandId)||null;
 ...
 async function loadLocations(){
  ...
  setWards((payload.wards||[]).map((x:any)=>({id:String(x.id),atoll_id:String(x.atoll_id),island_id:String(x.island_id),unit_type:String(x.unit_type||'WARD'),display_name:String(x.display_name||'')})).filter((x:LocationUnit)=>x.id&&x.island_id&&x.display_name));
 }
 ...
 function resetForm(){setEditingId(null);setLabel('Home');setHouse('');setRoad('');setAtollId('');setIslandId('');setWardId('');setAccessInstructions('');}
 function startAdd(){resetForm();setShowForm(true);setMessage('');}
 function startEdit(a:ServiceAddress){setEditingId(a.id);setLabel(a.label);setHouse(a.address_line1);setRoad(a.address_line2||'');setAtollId(a.service_atoll_id||'');setIslandId(a.service_island_id||'');setWardId(a.service_location_unit_id||'');setAccessInstructions(a.access_instructions||'');setShowForm(true);setMessage('');}
 function validate(){if(label.trim().length<2||label.trim().length>40)return'Name must be between 2 and 40 characters.';if(!selectedAtoll)return'Select an Atoll / Region.';if(!selectedIsland)return'Select an Island / City from the selected Atoll / Region.';if(filteredWards.length>0&&!selectedWard)return'Select a Ward.';if(road.trim().length<2||road.trim().length>120)return'Enter a valid Road / Street.';if(house.trim().length<2||house.trim().length>120)return'Enter a valid House / Apartment.';if(accessInstructions.trim().length>240)return'Access instructions must be 240 characters or fewer.';return'';}
 function body(){return{label:label.trim(),address_line1:house.trim(),address_line2:road.trim(),city:selectedIsland?.display_name||'',state_region:selectedAtoll?.display_name||'',postal_code:null,country:'Maldives',service_atoll_id:selectedAtoll?.id||null,service_island_id:selectedIsland?.id||null,service_location_unit_id:selectedWard?.id||null,access_instructions:accessInstructions.trim()||null};}
 ...
 function addressLine(a:ServiceAddress){const ward=wards.find(w=>w.id===a.service_location_unit_id)?.display_name;return[a.state_region,a.city,ward,a.address_line2,a.address_line1].filter(Boolean).join(' · ');}
 ...
 return <section className="serviceAddressManager" id="manage-service-addresses" aria-labelledby="service-address-manager-title">
  ...
  {showForm?<form className="samForm" onSubmit={save} noValidate><div className="samFormTitle"><strong>{editingId?'Edit Service Address':'Add Service Address'}</strong><button type="button" onClick={()=>{resetForm();setShowForm(false);setMessage('');}} disabled={busy}>Cancel</button></div><div className="samGrid">
   ...
   <label>Atoll / Region<select value={atollId} onChange={e=>{setAtollId(e.target.value);setIslandId('');setWardId('');}} disabled={busy} required><option value="">Select Atoll / Region</option>{atolls.map(a=><option key={a.id} value={a.id}>{a.display_name}</option>)}</select></label>
   <label>Island / City<select value={islandId} onChange={e=>{setIslandId(e.target.value);setWardId('');}} disabled={busy||!atollId} required><option value="">{atollId?'Select Island / City':'Select Atoll / Region first'}</option>{filteredIslands.map(i=><option key={i.id} value={i.id}>{i.display_name}</option>)}</select></label>
   {filteredWards.length?<label>Ward<select value={wardId} onChange={e=>setWardId(e.target.value)} disabled={busy||!islandId} required><option value="">Select Ward</option>{filteredWards.map(w=><option key={w.id} value={w.id}>{w.display_name}</option>)}</select></label>:null}
