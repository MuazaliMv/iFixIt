...
export default function ProfilePage(){
 ...
 async function load(){
  ...
  const{data:p,error}=await supabase.from('auth_profiles').select('role,full_name,email,provider_approved,created_at').eq('user_id',data.session.user.id).maybeSingle();
  if(error||!p){setMessage('Unable to load your profile.');return;}
  setProfile(p as Profile);
  try{const r=await fetch(REQUIREMENTS_API,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${data.session.access_token}`},body:JSON.stringify({action:'status'})});if(r.ok)setStatus(await r.json() as RequirementStatus);}catch{}
  setMessage('Profile is up to date.');
 }
 async function signOut(){await supabase.auth.signOut();window.location.href='/login';}
 const completeness=Math.max(0,Math.min(100,status?.profile_completeness??0));const initial=(profile?.full_name||profile?.email||'U').slice(0,1).toUpperCase();
 return <main className="shell accountApp">
  ...
  <button className="profileSignOut" type="button" onClick={signOut}>Sign Out</button><p className="muted" style={{textAlign:'center',fontSize:12}}>{message}</p>
 </main>;
}
