  ...
  <div className="profileRedesignShell">
   ...
   <div className="profileRedesignGrid">
    ...
    <div className="profileContentColumn">
     ...
     {serviceAddresses.length?<section className="profileInfoCard"><div><p className="profileInfoEyebrow">SAVED LOCATIONS</p><h2>Service addresses</h2></div><div className="profileLocationList">{serviceAddresses.map((s,i)=><div key={s.id||i}><span>{s.label||`Service location ${i+1}`}{s.is_default?' · Default':''}</span><strong>{[s.address_line1,s.address_line2,s.city,s.state_region,s.postal_code,s.country||'Maldives'].filter(Boolean).join(', ')||'Not provided'}</strong></div>)}</div></section>:null}
