...
async function profileWardRequest(){
 const{data,error}=await supabase.from('auth_profiles').select('ward').single();
 if(error)return '';
 return String(data?.ward||'');
}
...
export default function ProfileClient(){
 ...
 function populate(p:Profile,wardValue=ward){
  ...
  setName(p.full_name||'');setPhone(localPhone(p.phone_number));setLine1(a?.line1||'');setLine2(a?.line2||'');setCity(a?.city||'');setWard(wardValue||a?.ward||'');setStateRegion(a?.stateRegion||'');setPostalCode(a?.postalCode||'');setPostalChoices(a?.postalCode?[{postalCode:a.postalCode}]:[]);
 }
 ...
 async function load(){
  ...
  try{
   const[next,wardValue]=await Promise.all([profileRequest(),profileWardRequest()]);const normalized={...next,phone_number:localPhone(next.phone_number)};
   setProfile(normalized);populate(normalized,wardValue);setMessage('Profile is up to date.');setLoading(false);
 ...
 async function saveProfile(){
  ...
  try{
   const form=new FormData();form.set('fullName',trimmedName);form.set('phoneNumber',normalizedPhone);form.set('primaryAddress',JSON.stringify({line1:line1.trim()||null,line2:line2.trim()||null,city:city.trim()||null,stateRegion:stateRegion.trim()||null,postalCode:postalCode.trim()||null,country:'Maldives'}));
   ...
   if(profile?.user_id){const{error:wardError}=await supabase.from('auth_profiles').update({ward:ward.trim()||null}).eq('user_id',profile.user_id);if(wardError)throw wardError;}
   const next=await profileRequest();const normalized={...next,phone_number:localPhone(next.phone_number)};setProfile(normalized);populate(normalized,ward);if(normalized.role==='PROVIDER')void refreshProviderData();else setProviderData(null);setMessage('Profile updated successfully.');window.dispatchEvent(new Event('fixit:profile-updated'));
