  ...
  <div className="profileContentColumn">
   <section className="profileEditCard" id="profile"><form onSubmit={event=>{event.preventDefault();void saveProfile();}} className="profileEditForm">
    ...
    <div className="profileFormSection" id="service-addresses"><h3>Primary address <small>(optional)</small></h3><div className="profileFormGrid">
     ...
     <label>Island / City<select value={selectedIsland?.id||''} onChange={e=>{const next=availableIslands.find(i=>i.id===e.target.value);const nextCity=next?.display_name||'';setCity(nextCity);setWard('');setPostalCode('');setPostalChoices([]);const nextWards=next?wards.filter(w=>w.island_id===next.id):[];if(nextCity&&selectedAtoll&&!nextWards.length)void lookupPostalCode(nextCity,selectedAtoll.display_name,'');}} autoComplete="address-level2" disabled={loading||locationLookupLoading||!selectedAtoll}><option value="">{!selectedAtoll?'Not selected':locationLookupLoading?'Loading islands…':'Not selected'}</option>{availableIslands.map(i=><option key={i.id} value={i.id}>{i.display_name}</option>)}</select></label>
     <label>Ward<select value={selectedWard?.id||''} onChange={e=>{const next=availableWards.find(w=>w.id===e.target.value);const nextWard=next?.display_name||'';setWard(nextWard);setPostalCode('');setPostalChoices([]);if(city&&stateRegion)void lookupPostalCode(city,stateRegion,nextWard);}} disabled={loading||locationLookupLoading||!selectedIsland||!availableWards.length}><option value="">{!selectedIsland?'Not selected':availableWards.length?'Not selected':'No ward / not applicable'}</option>{availableWards.map(w=><option key={w.id} value={w.id}>{w.display_name}</option>)}</select></label>
