...
function remember(mode:Mode,message:string){
 try{localStorage.setItem('fixit:mobile-nav-role',mode);localStorage.setItem('fixit:app-mode',mode);sessionStorage.setItem('fixit:mode-toast',message);}catch{}
}
...
export default function AppModeSwitch({mode,compact=false,className=''}:Props){
 ...
 async function switchMode(){
  ...
  try{
   if(mode==='provider'){
    remember('customer',"You're now viewing as Customer. Providers won't see this view.");
    window.location.href='/';return;
   }
   const{data}=await supabase.auth.getSession();
   if(!data.session){window.location.href='/login';return;}
   if(providerReady===true){
    remember('provider',"You're now viewing as Provider. Customers won't see this view.");
    window.location.href='/provider';return;
   }
   remember('provider',"Provider setup is required before customers can book you.");
   window.location.href='/provider/onboarding?from=mode-switch';
  }finally{setBusy(false);}
 }
 const text=mode==='provider'?'Switch to Customer Mode':'Switch to Provider Mode';
 return <button type="button" className={`modeSwitch ${compact?'compact':''} ${className}`.trim()} onClick={()=>void switchMode()} disabled={busy} aria-label={text}>
  <span className={`modeDot ${mode}`}/>{compact?(mode==='provider'?'Customer':'Provider'):text}
 </button>;
}
