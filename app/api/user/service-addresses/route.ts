...
const FALLBACK_SUPABASE_KEY='sb_publishable_1sZEZgz9k2JACE_WzHtbCw_reiQEik6';
...
function supabaseConfig(){
 ...
 const key=process.env.SUPABASE_ANON_KEY?.trim()||process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY?.trim()||process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY?.trim()||FALLBACK_SUPABASE_KEY;
 if(!url||!key)throw new Error('Supabase public client configuration is incomplete.');
 return {url,key};
}
...
export async function POST(request:NextRequest){
 ...
 try{const client=userClient(auth.accessToken);const user=await currentUser(client,auth.accessToken);const admin=adminClient(auth.accessToken);const body=await request.json().catch(()=>({})) as Record<string,unknown>;const input=payloadFrom(body);const missing=validateAddress(input);if(missing.length)return applyAuthCookies(NextResponse.json({error:`Enter the missing Service Address records: ${missing.join(', ')}.`},{status:400}),auth);const count=await client.from('user_service_addresses').select('id',{count:'exact',head:true}).eq('user_id',user.id).eq('is_active',true);if(count.error)throw count.error;const result=await client.from('user_service_addresses').insert({...input,user_id:user.id,is_default:false}).select(ADDRESS_COLUMNS).single();if(result.error)throw result.error;let address=result.data;if((count.count||0)===0)address=await setDefault(client,admin,user.id,address.id);return applyAuthCookies(NextResponse.json({address},{status:201}),auth);}catch(error){return applyAuthCookies(NextResponse.json({error:error instanceof Error?error.message:'Unable to save Service Address.'},{status:500}),auth);}
}

export async function PATCH(request:NextRequest){
 if(!sameOrigin(request))return NextResponse.json({error:'Invalid request origin.'},{status:403});const auth=await resolveServerAuth(request);if(!auth)return NextResponse.json({error:'Authentication required.'},{status:401});
 try{const client=userClient(auth.accessToken);const user=await currentUser(client,auth.accessToken);const admin=adminClient(auth.accessToken);const body=await request.json().catch(()=>({})) as Record<string,unknown>;const id=clean(body.id);if(!id)return applyAuthCookies(NextResponse.json({error:'Service Address is required.'},{status:400}),auth);if(body.action==='set_default'){const address=await setDefault(client,admin,user.id,id);return applyAuthCookies(NextResponse.json({address}),auth);}const input=payloadFrom(body);const missing=validateAddress(input);if(missing.length)return applyAuthCookies(NextResponse.json({error:`Enter the missing Service Address records: ${missing.join(', ')}.`},{status:400}),auth);const existing=await client.from('user_service_addresses').select('is_default').eq('id',id).eq('user_id',user.id).eq('is_active',true).single();if(existing.error)throw new Error('Service Address not found.');const updated=await client.from('user_service_addresses').update(input).eq('id',id).eq('user_id',user.id).select(ADDRESS_COLUMNS).single();if(updated.error)throw updated.error;let address=updated.data;if(existing.data.is_default)address=await setDefault(client,admin,user.id,id);return applyAuthCookies(NextResponse.json({address}),auth);}catch(error){return applyAuthCookies(NextResponse.json({error:error instanceof Error?error.message:'Unable to update Service Address.'},{status:500}),auth);}
}

export async function DELETE(request:NextRequest){
 if(!sameOrigin(request))return NextResponse.json({error:'Invalid request origin.'},{status:403});const auth=await resolveServerAuth(request);if(!auth)return NextResponse.json({error:'Authentication required.'},{status:401});
 try{const client=userClient(auth.accessToken);const user=await currentUser(client,auth.accessToken);const admin=adminClient(auth.accessToken);const body=await request.json().catch(()=>({})) as Record<string,unknown>;const id=clean(body.id);if(!id)return applyAuthCookies(NextResponse.json({error:'Service Address is required.'},{status:400}),auth);const existing=await client.from('user_service_addresses').select('id,is_default').eq('id',id).eq('user_id',user.id).eq('is_active',true).single();if(existing.error)throw new Error('Service Address not found.');const removed=await client.from('user_service_addresses').update({is_active:false,is_default:false}).eq('id',id).eq('user_id',user.id);if(removed.error)throw removed.error;if(existing.data.is_default){const next=await client.from('user_service_addresses').select(ADDRESS_COLUMNS).eq('user_id',user.id).eq('is_active',true).order('updated_at',{ascending:false}).limit(1).maybeSingle();if(next.error)throw next.error;if(next.data)await setDefault(client,admin,user.id,next.data.id);else await syncDefault(admin,user.id,null);}return applyAuthCookies(NextResponse.json({removed:true}),auth);}catch(error){return applyAuthCookies(NextResponse.json({error:error instanceof Error?error.message:'Unable to remove Service Address.'},{status:500}),auth);}
}
