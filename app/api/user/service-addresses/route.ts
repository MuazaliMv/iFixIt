...
function supabaseConfig(){
 ...
 if(!key)throw new Error('Supabase authenticated-client configuration is incomplete.');
 return {url,key};
}
...
type ProfileAddressSync={
 default_service_address_id:string|null;
 ...
 country:string|null;
 primary_atoll_id:string|null;
 primary_island_id:string|null;
};
function profileAddressSync(address:any|null):ProfileAddressSync{
 return address?{
  default_service_address_id:address.id,
  ...
  country:'Maldives',
  primary_atoll_id:address.service_atoll_id||null,
  primary_island_id:address.service_island_id||null,
 }:{
...
async function syncDefault(client:ReturnType<typeof userClient>,userId:string,address:any|null){
 const profileValues=profileAddressSync(address);
