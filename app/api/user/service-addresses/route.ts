...
async function setDefault(client:ReturnType<typeof userClient>,admin:ReturnType<typeof adminClient>,userId:string,addressId:string){
 ...
 try{await syncDefault(admin,userId,chosen.data);}catch(error){await client.from('user_service_addresses').update({is_default:false}).eq('id',addressId).eq('user_id',userId);throw error;}
 return chosen.data;
}
...
export async function GET(request:NextRequest){
 ...
 try{const client=userClient(auth.accessToken);const result=await client.from('user_service_addresses').select(ADDRESS_COLUMNS).eq('user_id',auth.userId).eq('is_active',true).order('is_default',{ascending:false}).order('updated_at',{ascending:false});if(result.error)throw result.error;return applyAuthCookies(NextResponse.json({addresses:result.data||[]}),auth);}catch(error){return applyAuthCookies(NextResponse.json({error:error instanceof Error?error.message:'Unable to load Service Addresses.'},{status:500}),auth);}
}
...
export async function POST(request:NextRequest){
 ...
 try{const client=userClient(auth.accessToken);const admin=adminClient();const body=await request.json().catch(()=>({})) as Record<string,unknown>;const input=payloadFrom(body);const missing=validateAddress(input);if(missing.length)return applyAuthCookies(NextResponse.json({error:`Enter the missing Service Address records: ${missing.join(', ')}.`},{status:400}),auth);const existing=await client.from('user_service_addresses').select('id').eq('user_id',auth.userId).eq('is_active',true).limit(1);if(existing.error)throw existing.error;const result=await client.from('user_service_addresses').insert({...input,user_id:auth.userId,is_default:false}).select(ADDRESS_COLUMNS).single();if(result.error)throw result.error;let address=result.data;if((existing.data?.length||0)===0)address=await setDefault(client,admin,auth.userId,address.id);return applyAuthCookies(NextResponse.json({address},{status:201}),auth);}catch(error){return applyAuthCookies(NextResponse.json({error:error instanceof Error?error.message:'Unable to save Service Address.'},{status:500}),auth);}
}

export async function PATCH(request:NextRequest){
 if(!sameOrigin(request))return NextResponse.json({error:'Invalid request origin.'},{status:403});const auth=await resolveServerAuth(request);if(!auth)return NextResponse.json({error:'Authentication required.'},{status:401});
 try{const client=userClient(auth.accessToken);const admin=adminClient();const body=await request.json().catch(()=>({})) as Record<string,unknown>;const id=clean(body.id);if(!id)return applyAuthCookies(NextResponse.json({error:'Service Address is required.'},{status:400}),auth);if(body.action==='set_default'){const address=await setDefault(client,admin,auth.userId,id);return applyAuthCookies(NextResponse.json({address}),auth);}const input=payloadFrom(body);const missing=validateAddress(input);if(missing.length)return applyAuthCookies(NextResponse.json({error:`Enter the missing Service Address records: ${missing.join(', ')}.`},{status:400}),auth);const existing=await client.from('user_service_addresses').select('is_default').eq('id',id).eq('user_id',auth.userId).eq('is_active',true).single();if(existing.error)throw new Error('Service Address not found.');const updated=await client.from('user_service_addresses').update(input).eq('id',id).eq('user_id',auth.userId).select(ADDRESS_COLUMNS).single();if(updated.error)throw updated.error;let address=updated.data;if(existing.data.is_default)address=await setDefault(client,admin,auth.userId,id);return applyAuthCookies(NextResponse.json({address}),auth);}catch(error){return applyAuthCookies(NextResponse.json({error:error instanceof Error?error.message:'Unable to update Service Address.'},{status:500}),auth);}
}

export async function DELETE(request:NextRequest){
 if(!sameOrigin(request))return NextResponse.json({error:'Invalid request origin.'},{status:403});const auth=await resolveServerAuth(request);if(!auth)return NextResponse.json({error:'Authentication required.'},{status:401});
 try{const client=userClient(auth.accessToken);const admin=adminClient();const body=await request.json().catch(()=>({})) as Record<string,unknown>;const id=clean(body.id);if(!id)return applyAuthCookies(NextResponse.json({error:'Service Address is required.'},{status:400}),auth);const existing=await client.from('user_service_addresses').select('id,is_default').eq('id',id).eq('user_id',auth.userId).eq('is_active',true).single();if(existing.error)throw new Error('Service Address not found.');const removed=await client.from('user_service_addresses').update({is_active:false,is_default:false}).eq('id',id).eq('user_id',auth.userId);if(removed.error)throw removed.error;if(existing.data.is_default){const next=await client.from('user_service_addresses').select(ADDRESS_COLUMNS).eq('user_id',auth.userId).eq('is_active',true).order('updated_at',{ascending:false}).limit(1).maybeSingle();if(next.error)throw next.error;if(next.data)await setDefault(client,admin,auth.userId,next.data.id);else await syncDefault(admin,auth.userId,null);}return applyAuthCookies(NextResponse.json({removed:true}),auth);}catch(error){return applyAuthCookies(NextResponse.json({error:error instanceof Error?error.message:'Unable to remove Service Address.'},{status:500}),auth);}
}
