...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  try{const addressResponse=await rest(`user_service_addresses?select=id,label,address_line1,address_line2,city,state_region,postal_code,country,access_instructions,service_atoll_id,service_island_id,service_location_unit_id,is_default&user_id=eq.${encodeURIComponent(row.user_id)}&is_active=eq.true&order=updated_at.desc`,auth.authorization,3000);if(addressResponse.ok)serviceAddresses=await addressResponse.json().catch(()=>[]);}catch{}
  const profile={user_id:row.user_id,email:row.email,full_name:row.full_name,role:row.role,provider_approved:Boolean(row.provider_approved),phone_number:localProfilePhone(row.phone_number),is_phone_verified:Boolean(row.is_phone_verified),profile_photo_url:row.profile_photo_url,photoUrl:row.profile_photo_url,primaryAddress:{line1:row.address_line1||'',line2:row.address_line2||'',city:row.city||'',stateRegion:row.state_region||'',postalCode:row.postal_code||'',country:FIXED_COUNTRY},providerAddress:{line1:row.provider_address_line1||'',line2:row.provider_address_line2||'',city:row.provider_city||'',stateRegion:row.provider_state_region||'',postalCode:row.provider_postal_code||'',country:FIXED_COUNTRY},serviceAddresses:Array.isArray(serviceAddresses)?serviceAddresses:[],created_at:row.created_at};
