...
type ResolvedLocation={atollId:string;islandId:string};
...
async function resolvePrimaryLocation(client:ReturnType<typeof adminClient>,address:PrimaryAddress):Promise<ResolvedLocation>{
 const atollName=String(address.stateRegion||'').trim();const islandName=String(address.city||'').trim();
 ...
 return {atollId:atoll.data.id,islandId:island.data.id};
}
async function savePrimaryLocationIds(client:ReturnType<typeof adminClient>,accessToken:string,location:ResolvedLocation){
 ...
 const result=await client.from('auth_profiles').update({primary_atoll_id:location.atollId,primary_island_id:location.islandId}).eq('user_id',userResult.data.user.id);
...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const {data:addresses,error:addressError}=await client.from('user_service_addresses').select('id,label,address_line1,address_line2,city,ward,state_region,postal_code,country,service_atoll_id,service_island_id,access_instructions,is_default,is_active,created_at,updated_at').eq('user_id',userId).eq('is_active',true).order('is_default',{ascending:false}).order('updated_at',{ascending:false});
...
export async function PUT(request:NextRequest){
 ...
 try{
  ...
  if(contentType.includes('multipart/form-data')){
   ...
   // Saved Service Addresses have a dedicated authenticated API. Do not proxy
   // arbitrary address contracts through the profile Edge Function.
   form.delete('serviceAddresses');
   ...
   if('providerAddress' in body)body.providerAddress=forceMaldivesAddress(body.providerAddress);delete body.serviceAddresses;
