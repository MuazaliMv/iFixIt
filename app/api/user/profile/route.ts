...
async function rest(path:string,authorization:string){
 return fetch(`${SUPABASE_URL}/rest/v1/${path}`,{headers:{apikey:SUPABASE_PUBLISHABLE_KEY,Authorization:authorization},cache:'no-store'});
}
...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const profileResponse=await rest(profileUrl,authorization);
  ...
  try{
   const addressResponse=await rest(`user_service_addresses?select=id,label,address_line1,address_line2,city,state_region,postal_code,country,access_instructions,service_atoll_id,service_island_id,service_location_unit_id,is_default&user_id=eq.${encodeURIComponent(row.user_id)}&is_active=eq.true&order=updated_at.desc`,authorization);
  ...
  return NextResponse.json({error:error instanceof Error?error.message:'Unable to load your profile.'},{status:503});
 }
}
