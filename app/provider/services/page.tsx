...
function sameSelection(a:string[],b:string[]){
 ...
 const aa=[...a].sort();const bb=[...b].sort();return aa.every((id,index)=>id===bb[index]);
}
...
function sameAreas(a:Area[],b:Area[]){
 if(a.length!==b.length)return false;
 const aa=a.map(areaKey).sort();const bb=b.map(areaKey).sort();return aa.every((id,index)=>id===bb[index]);
}
...
export default function ProviderServicesPage(){
 ...
 useEffect(()=>{
  if(mode.ready){
   ...
   const current=(mode.serviceAreas||[]).map((a:any)=>({
    islandId:a.islandId,locationUnitId:a.locationUnitId||null,islandName:a.islandName||'Selected island',locationUnitName:a.locationUnitName||null,
   }));
 ...
 const visibleAreas=useMemo(()=>{
  ...
  return areas.filter(a=>!q||`${a.islandName} ${a.locationUnitName||'whole island'}`.toLowerCase().includes(q));
 },[areas,locationQuery]);
 ...
 async function loadLocations(){
  ...
  try{
   const{data}=await supabase.auth.getSession();if(!data.session)throw new Error('Sign in required');
   ...
   const p=await r.json();if(!r.ok)throw new Error(p?.error||'Unable to load service locations');
   setAtolls(p.atolls||[]);setIslands(p.islands||[]);setUnits(p.locationUnits||[]);
  }catch(e){setMessage(e instanceof Error?e.message:'Unable to load service locations.');}
  finally{setLocationsLoading(false);}
 }

 function addArea(){
  const island=islands.find(i=>i.id===islandId);if(!island){setMessage('Choose an island or city first.');return;}
  ...
  setAreas(v=>[next,...v]);setUnitId('');setLocationPage(1);setMessage('Service location added. Save changes to apply it.');
 }
 function removeArea(key:string){
  setAreas(v=>v.filter(a=>areaKey(a)!==key));
  if(openLocationKey===key)setOpenLocationKey(null);
  setMessage('Service location removed. Save changes to apply it.');
 }

 async function save(){
  if(!hasChanges||busy)return;setBusy(true);setMessage('');
  try{
   const{data}=await supabase.auth.getSession();if(!data.session)throw new Error('Sign in required');
   ...
   const out=await r.json();if(!r.ok)throw new Error(out?.error||'Unable to save services and locations');
   setSavedSelected(selected);setSavedAreas(areas);await mode.reload();setMessage('Services and service locations saved.');
 ...
 return <main className="providerServicesPage">
  <div className="providerServicesShell">
   <header className="providerServicesHeader">
    ...
    <div className="providerServicesHeading"><h1>Manage Services</h1><p>Choose the services you offer and the locations where they are available.</p></div>
   ...
   <section className="providerServicesSection" aria-labelledby="active-services-title">
    <div className="providerServicesSectionHead"><h2 id="active-services-title">Your active services</h2><span>{selected.length} selected</span></div>
    <div className="providerServicesGrid">
     {categories.map(c=>{const isSelected=selected.includes(c.id);return <button type="button" key={c.id} className={`providerServiceTile${isSelected?' selected':''}`} aria-pressed={isSelected} onClick={()=>toggle(c.id)}><span className="providerServiceIcon" aria-hidden="true">{SERVICE_ICONS[c.name]||'⚒'}</span><span className="providerServiceName">{c.name}</span><span className="providerServiceCheck" aria-hidden="true">{isSelected?'✓':''}</span></button>;})}
    </div>
   </section>

   <section className="providerServicesSection providerLocationsSection" aria-labelledby="service-locations-title">
    <div className="providerServicesSectionHead"><div><h2 id="service-locations-title">Manage service locations</h2><p>Use the same card-style workflow as Request Management.</p></div><span>{areas.length} active</span></div>

    <div className="providerLocationToolbar">
     <div className="providerLocationStatus"><span className="providerLocationStatusIcon">✓</span><span><b>{areas.length} service location{areas.length===1?'':'s'}</b><small>{hasChanges?'Unsaved changes are ready to save.':'All location changes are saved.'}</small></span></div>
     <label className="providerLocationSearch">Search locations<input value={locationQuery} onChange={e=>setLocationQuery(e.target.value)} placeholder="Island, city, ward or area" /></label>
     <div className="providerSetupForm providerLocationAddForm">
      <label>Atoll<select value={atollId} disabled={locationsLoading||!atolls.length} onChange={e=>{setAtollId(e.target.value);setIslandId('');setUnitId('');}}><option value="">{locationsLoading?'Loading atolls…':atolls.length?'Select atoll':'No atolls available'}</option>{atolls.map(a=><option key={a.id} value={a.id}>{a.display_name}</option>)}</select></label>
      <label>Island / City<select value={islandId} disabled={!atollId||!islandOptions.length} onChange={e=>{setIslandId(e.target.value);setUnitId('');}}><option value="">{!atollId?'Select atoll first':islandOptions.length?'Select island / city':'No islands available'}</option>{islandOptions.map(i=><option key={i.id} value={i.id}>{i.display_name}</option>)}</select></label>
      {unitOptions.length?<label>Ward / Area<select value={unitId} onChange={e=>setUnitId(e.target.value)}><option value="">Whole island</option>{unitOptions.map(u=><option key={u.id} value={u.id}>{u.display_name}</option>)}</select></label>:null}
      <button className="providerLocationAddButton" type="button" disabled={!islandId} onClick={addArea}>+ Add location</button>
     </div>
    </div>

    <div className="providerLocationList">
     {pagedAreas.map(a=>{
      const key=areaKey(a);const isSaved=savedAreas.some(s=>areaKey(s)===key);const open=openLocationKey===key;
      return <div className="providerLocationCardGroup" key={key}>
       <article className={`providerLocationCard ${isSaved?'saved':'pending'}`}>
        <div className="providerLocationCardTop"><div><strong>{a.islandName}</strong><span>{a.locationUnitName||'Whole island'}</span></div><span className={`providerLocationBadge ${isSaved?'saved':'pending'}`}>{isSaved?'Saved':'Pending save'}</span></div>
        <div className="providerLocationMeta"><span><b>Coverage</b>{a.locationUnitName?'Specific area':'Whole island'}</span><span><b>Availability</b>Customers can request active services here</span></div>
        <div className="providerLocationActions"><button className="providerLocationOpenButton" type="button" onClick={()=>setOpenLocationKey(open?null:key)}>{open?'Close Details':'Open Location'}</button></div>
       </article>
       {open?<div className="providerLocationDetail"><div><p className="providerLocationEyebrow">SERVICE LOCATION DETAIL</p><h3>{a.islandName}</h3><p>{a.locationUnitName||'Whole island coverage'}</p></div><div className="providerLocationDetailActions"><span className={`providerLocationBadge ${isSaved?'saved':'pending'}`}>{isSaved?'Active after last save':'Will activate when saved'}</span><button className="providerLocationRemoveButton" type="button" onClick={()=>removeArea(key)}>Remove location</button></div></div>:null}
      </div>;
     })}
     {!visibleAreas.length?<div className="providerLocationEmpty">No service locations match your search.</div>:null}
    </div>
  ...
  <div className="providerServicesBottomBar"><div className="providerServicesBottomInner"><button className="providerServicesSaveButton" style={{width:'100%',gridColumn:'1 / -1'}} disabled={busy||!hasChanges} onClick={()=>void save()}><span aria-hidden="true">▣</span>{busy?'Saving…':'Save services & locations'}</button></div></div>
 </main>;
}
