...
const LOCATION_URL='/api/legacy-edge?service=location-catalogue';
const PAGE_SIZE=10;
...
export default function ProviderServicesPage(){
 ...
 const[selected,setSelected]=useState<string[]>([]);const[savedSelected,setSavedSelected]=useState<string[]>([]);
 const[atolls,setAtolls]=useState<Atoll[]>([]);const[islands,setIslands]=useState<Island[]>([]);const[units,setUnits]=useState<Unit[]>([]);
 const[areas,setAreas]=useState<Area[]>([]);const[savedAreas,setSavedAreas]=useState<Area[]>([]);
 const[availability,setAvailability]=useState('BY_APPOINTMENT');const[savedAvailability,setSavedAvailability]=useState('BY_APPOINTMENT');
 const[hours,setHours]=useState<Hour[]>([]);const[savedHours,setSavedHours]=useState<Hour[]>([]);
 const[atollId,setAtollId]=useState('');const[islandId,setIslandId]=useState('');const[unitId,setUnitId]=useState('');
 const[locationQuery,setLocationQuery]=useState('');const[locationPage,setLocationPage]=useState(1);const[locationsLoading,setLocationsLoading]=useState(true);const[busy,setBusy]=useState(false);const[message,setMessage]=useState('');

 useEffect(()=>{if(mode.ready){setSelected(mode.selectedCategoryIds);setSavedSelected(mode.selectedCategoryIds);const current=(mode.serviceAreas||[]).map((a:any)=>({islandId:a.islandId,locationUnitId:a.locationUnitId||null,islandName:a.islandName||'Selected island',locationUnitName:a.locationUnitName||null}));setAreas(current);setSavedAreas(current);const currentAvailability=mode.profile?.availability_status||'BY_APPOINTMENT';setAvailability(currentAvailability);setSavedAvailability(currentAvailability);const currentHours=days.map((_,i)=>{const h=mode.hours.find((x:any)=>Number(x.day_of_week)===i+1);return{dayOfWeek:i+1,isWorking:Boolean(h?.is_working),startTime:String(h?.start_time||'08:00').slice(0,5),endTime:String(h?.end_time||'17:00').slice(0,5)}});setHours(currentHours);setSavedHours(currentHours);}},[mode.ready,mode.selectedCategoryIds,mode.serviceAreas,mode.hours,mode.profile]);
 useEffect(()=>{void loadLocations();},[]);

 const islandOptions=useMemo(()=>islands.filter(i=>i.atoll_id===atollId),[islands,atollId]);const unitOptions=useMemo(()=>units.filter(u=>u.island_id===islandId),[units,islandId]);
 ...
 const locationPages=Math.max(1,Math.ceil(visibleAreas.length/PAGE_SIZE));const pagedAreas=useMemo(()=>visibleAreas.slice((locationPage-1)*PAGE_SIZE,locationPage*PAGE_SIZE),[visibleAreas,locationPage]);
 useEffect(()=>{setLocationPage(1);},[locationQuery]);useEffect(()=>{if(locationPage>locationPages)setLocationPage(locationPages);},[locationPage,locationPages]);

 async function loadLocations(){setLocationsLoading(true);try{const p=await post(LOCATION_URL,{});setAtolls(p.atolls||[]);setIslands(p.islands||[]);setUnits(p.locationUnits||[]);}catch(e){if(!(e instanceof Error&&e.message==='Authentication required.'))setMessage(e instanceof Error?e.message:'Unable to load service locations.');}finally{setLocationsLoading(false);}}
 ...
 function removeArea(key:string){setAreas(v=>v.filter(a=>areaKey(a)!==key));setMessage('Service location removed. Save changes to apply it.');}
 function patchHour(day:number,p:Partial<Hour>){setHours(v=>v.map(h=>h.dayOfWeek===day?{...h,...p}:h));}

 async function save(){if(!hasChanges||busy)return;setBusy(true);setMessage('');try{if(!selected.length)throw new Error('Keep at least one active service.');if(!areas.length)throw new Error('Keep at least one service location.');const p=mode.profile||{};await post(ONBOARDING_URL,{action:'save',providerType:p.provider_type||'INDIVIDUAL',publicName:p.public_name||mode.name,businessName:p.business_name||'',description:p.description||'',experienceYears:Number(p.experience_years||0),availabilityStatus:availability,categoryIds:selected,hours,serviceAreas:areas.map(a=>({islandId:a.islandId,locationUnitId:a.locationUnitId||null}))});setSavedSelected(selected);setSavedAreas(areas);setSavedAvailability(availability);setSavedHours(hours);await mode.reload();setMessage('Services, locations and availability saved.');}catch(e){if(!(e instanceof Error&&e.message==='Authentication required.'))setMessage(e instanceof Error?e.message:'Unable to save provider settings.');}finally{setBusy(false);}}
 ...
 const categories:Category[]=mode.categories;const toggle=(id:string)=>setSelected(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id]);
 ...
 return <main className="providerServicesPage"><div className="providerServicesShell">
  ...
  <section className="providerServicesSection providerLocationsSection" aria-labelledby="service-locations-title"><div className="providerServicesSectionHead"><div><h2 id="service-locations-title">Manage service locations</h2><p>Choose the islands, cities, wards or areas where customers can request your services.</p></div><span>{areas.length} active</span></div>
   ...
   <div className="providerLocationList">{pagedAreas.map(a=>{const key=areaKey(a);const isSaved=savedAreas.some(s=>areaKey(s)===key);return <article className={`providerLocationCard ${isSaved?'saved':'pending'}`} key={key}><div className="providerLocationCardTop"><div><strong>{a.islandName}</strong><span>{a.locationUnitName||'Whole island'}</span></div><div className="providerLocationCardControls"><span className={`providerLocationBadge ${isSaved?'saved':'pending'}`}>{isSaved?'Saved':'Pending save'}</span><button className="providerLocationQuickRemove" type="button" aria-label={`Remove ${a.islandName}`} title="Remove location" onClick={()=>removeArea(key)}>×</button></div></div><div className="providerLocationMeta"><span><b>Coverage</b>{a.locationUnitName?'Specific area':'Whole island'}</span><span><b>Availability</b>Customers can request active services here</span></div></article>;})}{!visibleAreas.length?<div className="providerLocationEmpty">No service locations match your search.</div>:null}</div>{visibleAreas.length>PAGE_SIZE?<div className="providerLocationPagination"><button type="button" disabled={locationPage===1} onClick={()=>setLocationPage(p=>Math.max(1,p-1))}>Previous</button><span>Page {locationPage} of {locationPages} · {visibleAreas.length} locations</span><button type="button" disabled={locationPage===locationPages} onClick={()=>setLocationPage(p=>Math.min(locationPages,p+1))}>Next</button></div>:null}
  </section>
