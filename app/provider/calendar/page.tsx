...
export default function ProviderCalendarPage(){
 ...
 useEffect(()=>{if(!state.ready)return;void(async()=>{try{const r=await fetch(MARKET_URL,{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'dashboard'})});if(r.status===401){window.location.replace(`/login?next=${encodeURIComponent('/provider/calendar')}`);return;}const p=await r.json().catch(()=>({}));if(r.ok)setJobs(p.requests||[]);}catch{}})();},[state.ready]);
 const upcoming=useMemo(()=>jobs.filter(j=>Boolean(j.inspection?.scheduled_start)&&String(j.status).toUpperCase()!=='COMPLETED').sort((a,b)=>new Date(a.inspection?.scheduled_start||0).getTime()-new Date(b.inspection?.scheduled_start||0).getTime()),[jobs]);
