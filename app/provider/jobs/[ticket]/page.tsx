...
function formatPreferredSlot(value:string){
 ...
 if(!Number.isNaN(d.getTime())&&/(Z|[+-]\d{2}:\d{2})$/.test(value))return d.toLocaleString();
 const local=new Date(value.length===16?`${value}:00`:value);
 return Number.isNaN(local.getTime())?value:local.toLocaleString();
}
...
export default function ProviderJobDetailPage(){
 ...
 async function act(action:'schedule'|'start'|'complete'){
  setBusy(true);
  try{
   const s=await session();if(!s)return;
   const r=await fetch(FLOW_URL,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${s.access_token}`},body:JSON.stringify({action,ticketNumber:ticket,scheduledStart:schedule||undefined})});
   const p=await r.json();
   if(!r.ok)throw new Error(p?.error||'Unable to update job');
   setMessage(action==='schedule'?'Visit time saved.':action==='start'?'Work started.':'Job completed. Waiting for customer confirmation.');
   await load();
  }catch(e){
   setMessage(e instanceof Error?e.message:'Unable to update job.');
  }finally{setBusy(false);}
 }
 ...
 const workingInternal=job?.status==='IN_PROGRESS';
 const closed=stage==='CUSTOMER CONFIRMED';
 const preferredSlots=(job?.inspection?.preferred_slots||[]).filter(Boolean);
 ...
 return <main className="providerModePage"><div className="providerModeShell"><header className="providerModeTop"><div><span className="modeBadge provider"><span className="modeDot provider"/>Provider</span><h1>{job?.service_name||'Customer Job'}</h1><p>{ticket}</p></div><AppModeSwitch mode="provider" compact/></header>{job?<>
  ...
  <section className="providerModeCard providerNextAction"><div className="providerSectionHead"><div><h2>{closed?'Job closed':'Next action'}</h2><p>{stage==='ACCEPTED'?'The customer selected you. Confirm this job to continue.':stage==='CONFIRMED'?'Choose one of the customer preferred times below, or set another agreed visit time.':stage==='IN PROGRESS'&&scheduledInternal?'The visit time is saved. Start the work when ready.':stage==='IN PROGRESS'&&workingInternal?'Work is active. Mark the job complete when finished.':stage==='COMPLETED'?'Work is complete. Waiting for the customer to confirm completion.':'No further provider action is required.'}</p></div></div>
   ...
   {stage==='IN PROGRESS'&&workingInternal?<button className="success providerPrimaryAction" disabled={busy} onClick={()=>void act('complete')}>Mark Complete</button>:null}
  ...
  <section className="providerJobColumns">
   <div className="providerModeCard"><div className="providerSectionHead"><div><h2>Customer</h2><p>{contactUnlocked?'Contact details are available.':'Contact details unlock after customer confirmation.'}</p></div></div>{contactUnlocked?<><div className="providerContactIdentity"><strong>{job.customer?.name||'Customer'}</strong><span>{job.customer?.phone||'Phone not provided'}</span></div><div className="providerContactActions">{job.customer?.phone?<a className="secondary" href={`tel:${job.customer.phone}`}>Call customer</a>:null}<a className="secondary" href={`/provider/messages?ticket=${encodeURIComponent(ticket)}`}>Message</a></div></>:<div className="providerLockedContact">🔒 Customer contact is available after confirmation.</div>}</div>
   <div className="providerModeCard"><div className="providerSectionHead"><div><h2>On-site contact</h2><p>{contactUnlocked?'Use this person for arrival and access.':'On-site contact unlocks after customer confirmation.'}</p></div></div>{contactUnlocked&&job.onSiteContact?<><div className="providerContactIdentity"><strong>{job.onSiteContact.name||'On-site contact'}</strong><span>{job.onSiteContact.sameAsCustomer?'Same as customer':job.onSiteContact.phone||'Phone not provided'}</span></div><div className="providerContactActions">{job.onSiteContact.phone?<a className="secondary" href={`tel:${job.onSiteContact.phone}`}>Call on-site</a>:null}<a className="secondary" href={directions} target="_blank" rel="noreferrer">Directions</a></div></>:<div className="providerLockedContact">🔒 On-site contact is available after confirmation.</div>}</div>
  </section>
  <section className="providerJobColumns">
   <div className="providerModeCard"><div className="providerSectionHead"><div><h2>Visit</h2></div></div><dl className="providerReadableData"><div><dt>Location</dt><dd>{job.service_location_text}</dd></div><div><dt>Preferred date</dt><dd>{job.preferred_date?new Date(job.preferred_date+'T00:00:00').toLocaleDateString():'Not specified'}</dd></div><div><dt>Customer preferred times</dt><dd>{preferredSlots.length?<div>{preferredSlots.map((slot,i)=><div key={`${slot}-visit-${i}`}>{formatPreferredSlot(slot)}</div>)}</div>:'Not submitted'}</dd></div><div><dt>Visit time</dt><dd>{job.inspection?.scheduled_start?new Date(job.inspection.scheduled_start).toLocaleString():'Not scheduled'}</dd></div></dl></div>
   <div className="providerModeCard"><div className="providerSectionHead"><div><h2>Job</h2></div></div><dl className="providerReadableData"><div><dt>Service</dt><dd>{job.service_name}</dd></div><div><dt>Reference</dt><dd>{ticket}</dd></div></dl>{job.problem_description?<div className="providerJobProblem providerProblemClean"><small>Customer request</small><p>{job.problem_description}</p></div>:null}</div>
  </section>
