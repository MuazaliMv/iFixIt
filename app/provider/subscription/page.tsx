...
const API='/api/legacy-edge?service=provider-subscription';
...
export default function ProviderSubscriptionPage(){
 ...
 useEffect(()=>{void load();},[]);
 async function post(action:string){const r=await fetch(API,{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({action})});const p=await r.json().catch(()=>({}));if(r.status===401){window.location.replace(`/login?next=${encodeURIComponent('/provider/subscription')}`);throw new Error('Authentication required.');}if(!r.ok)throw new Error(p?.error||'Unable to load subscription');return p;}
 async function load(){setLoading(true);try{const p=await post('status');setSub(p?.subscription||null);setNotice('');}catch(e){if(!(e instanceof Error&&e.message==='Authentication required.'))setNotice(e instanceof Error?e.message:'Unable to load subscription.');}finally{setLoading(false);}}
 async function subscribe(){setBusy(true);setNotice('');try{const p=await post('start_payment');if(p?.checkoutUrl)window.location.href=p.checkoutUrl;else setNotice('Payment gateway is not ready yet.');}catch(e){if(!(e instanceof Error&&e.message==='Authentication required.'))setNotice(e instanceof Error?e.message:'Unable to start payment.');}finally{setBusy(false);}}
