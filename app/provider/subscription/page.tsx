...
export default function ProviderSubscriptionPage(){
 ...
 async function logout(){try{localStorage.removeItem('fixit:mobile-nav-role');localStorage.removeItem('fixit:app-mode');localStorage.removeItem('fixit:mode-toast');}catch{}await supabase.auth.signOut();window.location.href='/login';}
 ...
 return <main className="providerModePage"><div className="providerModeShell">
  <header className="providerModeTop"><div><span className="modeBadge provider"><span className="modeDot provider"/>Provider</span><h1>Subscription</h1><p>Keep provider access active and continue receiving customer requests.</p></div><div style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:10,flexWrap:'wrap'}}><AppModeSwitch mode="provider" compact/><button className="secondary" type="button" onClick={()=>void logout()}>Log Out</button></div></header>
  {sub?<section className="providerModeCard"><div className="providerModeHero"><div><span className={`modeBadge ${sub.active?'provider':'customer'}`}>{sub.status==='TRIAL'?'FREE TRIAL':sub.status}</span><h2>{sub.status==='EXPIRED'?'Your subscription has expired.':sub.status==='TRIAL'?`${sub.daysRemaining} day${sub.daysRemaining===1?'':'s'} of free access remaining`:'Subscription active'}</h2><p>{sub.status==='EXPIRED'?'Please renew to continue using the provider dashboard and service listings.':`Access is available until ${expiry}.`}</p></div>{sub.active?<a className="secondary" href="/provider/today">Provider Dashboard</a>:null}</div></section>:null}
  <section className="providerModeCard"><div className="providerSectionHead"><div><h2>Monthly provider plan</h2><p>One simple plan for provider access.</p></div><strong style={{fontSize:24}}>250 MVR <small style={{fontSize:13,fontWeight:600}}>/ 30 days</small></strong></div><div className="providerReadableData" style={{marginTop:16}}><div><dt>Free trial</dt><dd>30 days for new providers</dd></div><div><dt>Renewal period</dt><dd>30 days per successful payment</dd></div><div><dt>Payment gateway</dt><dd>Bank of Maldives (BML)</dd></div></div>{sub?.status==='TRIAL'?<p className="muted" style={{marginTop:16}}>Your free trial ends on {expiry}. You do not need to pay until the trial expires.</p>:null}{sub?.status==='ACTIVE'?<p className="muted" style={{marginTop:16}}>Current access ends on {expiry}. Early renewal will add 30 days after the current expiry date.</p>:null}{sub?.status==='EXPIRED'?<button className="primary providerPrimaryAction" style={{marginTop:18}} disabled={busy} onClick={()=>void subscribe()}>{busy?'Opening payment…':'Subscribe Now – 250 MVR'}</button>:null}</section>
